...
def get_raw_scores(examples, preds):
    ...
    for example in examples:
        ...
        if type(example.answers) == str:
            gold_answers = [normalize_answer(example.answers)]
        elif type(example.answers) == list:
...
def make_eval_dict_kat(exact_scores, f1_scores):
    # Merge evaluations
    # key to short key
    key_to_short = dict()
    for key in exact_scores.keys():
        a, b, c = key.split('_')
        value = f"{a}_{b}"
        key_to_short[key] = value
    
    new_exact_scores = {
        key: [] for key in key_to_short.values()
    }
    new_f1_scores = {
        key: [] for key in key_to_short.values()
    }
    ...
    for key in exact_scores.keys():
        new_exact_scores[key_to_short[key]].append(exact_scores[key])
        new_f1_scores[key_to_short[key]].append(f1_scores[key])

    exact_scores = dict()
    f1_scores = dict()
    for k, v in new_exact_scores.items():
        exact_scores[k] = max(v)
    for k, v in new_f1_scores.items():
        f1_scores[k] = max(v)
...
def squad_evaluate(examples, preds, no_answer_probs=None, no_answer_probability_threshold=1.0):
    ...
    evaluation = make_eval_dict_kat(exact_threshold, f1_threshold)

    return evaluation, exact_threshold, f1_threshold
